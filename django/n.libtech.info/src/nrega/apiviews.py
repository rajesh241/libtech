...
def crawlDataForPt(request):
    ...
    inQueueCount=len(CrawlQueue.objects.filter(panchayat=myPanchayat,isComplete=False))
    if inQueueCount == 0:
        CrawlQueue.objects.create(panchayat=myPanchayat,priority=5000)
    else:
        pass
...
def PanchayatReport(request):
    ptid = request.GET.get('ptid', '')
    report = Report.objects.filter(panchayat == ptid)
...
def crawlDataRequest(request):
    ...
    if len(code) == 7:
      myBlock=Block.objects.filter(code=code).first()
      CrawlQueue.objects.create(block=myBlock,priority=5000)
    elif len(code) == 10:
...
def crawlStatusPt(request):
    ptid=request.GET.get('ptid', '')
    if ptid == '':
        crawlStatus = CrawlQueue.objects.filter(isComplete = True)
    else:
        myPanchayat=Panchayat.objects.filter(id=ptid).first()
        crawlStatus = CrawlQueue.objects.filter(panchayat = myPanchayat)
...
def getStates(request):
    ...
    if request.method == 'GET':
        ...
        curfinyear='18'
        totalTransactions=PanchayatStat.objects.filter(panchayat__block__district__state__name=state,finyear=curfinyear,workDaysAccuracyIndex__gte = 90).values ('panchayat__block__district__state__name', 'panchayat__block__name').annotate(state = F('panchayat__block__district__state__name'), block = F('panchayat__block__name'), transactions = Count('id'))
        serializer = StateSerializer(totalTransactions, many=True)
        return JsonResponse(serializer.data, safe=False)
...
def jobcardInfo(request):
  ...
  if status == '':
    ...
    wds=WorkDetail.objects.filter(worker__jobcard__jobcard=jobcard,musterStatus=status).order_by("-muster__dateTo")

  serializer =WorkDetailSerializer5(wds, many=True)
  return JsonResponse(serializer.data, safe=False)
...
def getPaymentInfo(request):
  ...
  if pid == '':
    pid=72447
  paymentinfos=PaymentInfo.objects.filter(id=pid)
  serializer =PaymentInfoSerializer(paymentinfos, many=True)
  return JsonResponse(serializer.data, safe=False)

@csrf_exempt
def jobcardInfo1(request):
  """
  Get Details about each Jobcard
  """
  if request.method=='GET':
    ...
    status = request.GET.get('status','')
  if status == '':
    paymentinfos=PaymentInfo.objects.filter(workDetail__worker__jobcard__jobcard=jobcard).order_by("-workDetail__muster__dateTo")
  else:
    paymentinfos=PaymentInfo.objects.filter(workDetail__worker__jobcard__jobcard=jobcard,workDetail__musterStatus=status).order_by("-workDetail__muster__dateTo")
...
def getWagelists(request):
  ...
  if bcode == '':
    ...
    wagelists=Wagelist.objects.filter(block__code=bcode)[:limit]
  serializer =WagelistSerializer(wagelists, many=True)
  return JsonResponse(serializer.data, safe=False)
