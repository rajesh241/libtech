...
def createExtendedPPReport(logger,eachPanchayat,finyear,eachBlock=None):
  f = BytesIO()
  f.write(u'\ufeff'.encode('utf8'))
  w = csv.writer(f, encoding='utf-8-sig',delimiter=',')
  #w = csv.writer(f, newline='',delimiter=',')
  reportType="extendedPPReport"
#  logger.info("Createing extended Rejected Payment ReportsPayment for panchayat: %s panchayatCode: %s ID: %s" % (eachPanchayat.name,eachPanchayat.code,str(eachPanchayat.id)))
  a=[]
#  workRecords=WorkDetail.objects.filter(id=16082209)
  if eachBlock is not None:
    ...
    if finyear is not None:
      workRecords=WorkDetail.objects.filter( Q ( Q(worker__jobcard__panchayat__block=eachBlock,muster__finyear=finyear) & Q ( Q(musterStatus='') )      ) ).order_by("worker__jobcard__panchayat__name","worker__jobcard__village__name","worker__jobcard__jcNo")
    else:
      workRecords=WorkDetail.objects.filter( Q ( Q(worker__jobcard__panchayat__block=eachBlock) & Q ( Q(musterStatus='Rejected') | Q (musterStatus="Invalid Account") )     ) ).order_by("worker__jobcard__panchayat__name","worker__jobcard__village__name","worker__jobcard__jcNo")
  else:
    ...
    w.writerow(a)
    if finyear is not None:
      workRecords=WorkDetail.objects.filter( Q ( Q(worker__jobcard__panchayat=eachPanchayat,muster__finyear=finyear) & Q ( Q (musterStatus="") )     ) ).order_by("worker__jobcard__village__name","worker__jobcard__jcNo")
    else:
      workRecords=WorkDetail.objects.filter( Q ( Q(worker__jobcard__panchayat=eachPanchayat) & Q ( Q(musterStatus='Rejected') | Q (musterStatus="Invalid Account") )     ) ).order_by("worker__jobcard__village__name","worker__jobcard__jcNo")
  logger.info("Total Work Records: %s " %str(len(workRecords)))
  for wd in workRecords:
    workName=wd.muster.workName
    applicantName=wd.worker.name
...
#    shutil.copyfileobj(f, f1)
  outcsv=f.getvalue()
  if eachBlock is not None:
    csvfilename=eachBlock.slug+"_"+finyear+"_extdPenPayment.csv"
    saveBlockReport(logger,eachBlock,finyear,reportType,csvfilename,outcsv)
  else:
    csvfilename=eachPanchayat.slug+"_"+finyear+"_extdPenPayment.csv"
    savePanchayatReport(logger,eachPanchayat,finyear,reportType,csvfilename,outcsv)
