...
from stateShortCodes import stateShortCodeDict
    ...
    if "BIHAR" in str(table):
       ...
       for link in links:
         ...
         if "state_name" in str(link):
           ...
           if mystate is None:
             State.objects.create(name=stateName,code=stateCode)
           mystate=State.objects.filter(code=stateCode).first()
           stateShortCode=stateShortCodeDict[stateCode]
           if stateShortCode is not None:
             mystate.stateShortCode=stateShortCode
           logger.info("State Short Code is %s " % stateShortCode)
           mystate.name=stateName
           mystate.crawlIP=crawlIP
           mystate.save()
           states=State.objects.filter(code=stateCode)
           mystate=states.first()

           r=requests.get(stateURL)
           try:
             r= requests.get(enlance, verify=False, timeout=5)
           ...
           if r is not None:
             ...
             if table is not None:
               ...
               for distLink in distLinks:
                 ...
                 districts=District.objects.filter(code=fullDistrictCode)
                 if len(districts)==0:
                   logger.info("No District Found")
                   District.objects.create(name=rawDistrictName,code=fullDistrictCode,state=mystate)
                 else:
