...
def main():
  ...
  allDistricts=District.objects.filter(isEnumerated=False)[:50]
  if args['stateCode']:
    allDistricts=District.objects.filter(state__code=args['stateCode'])
  #allDistricts=District.objects.filter(name="surguja")
  for eachDistrict in allDistricts:
    districtName=eachDistrict.name
    fullDistrictCode=eachDistrict.code
    stateName=eachDistrict.state.name
    crawlIP=eachDistrict.state.crawlIP
    stateCode=eachDistrict.state.code
      ...
      if table is not None:
        ...
        for row in rows:
          ...
          for column in columns:
            ...
            blocks=Block.objects.filter(code=fullBlockCode)
            if len(blocks)==0:
              logger.info("No Block Found")
              Block.objects.create(name=rawBlockName,code=fullBlockCode,district=eachDistrict)
            else:
              ...
              if noPanchayat == 0:
                ...
                if len(allPanchayatLinks) > 0:
                  for eachPanchayat in allPanchayatLinks:
                    ...
                    panchayats=Panchayat.objects.filter(code=fullPanchayatCode)
                    if len(panchayats)==0:
                      logger.info("No Panchayat Found")
                      Panchayat.objects.create(name=rawPanchayatName,code=fullPanchayatCode,block=myblock)
                    else:
    ...
    eachDistrict.isEnumerated=True
    eachDistrict.save()
