...
from customSettings import repoDir,djangoDir,djangoSettings,startFinYear,panchayatCrawlThreshold,panchayatRetryThreshold,telanganaStateCode,panchayatAttemptRetryThreshold
...
def argsFetch():
  ...
  parser.add_argument('-b', '--blockCode', help='Panchayat for which the delayed payment report needs to be crawld', required=False)
  ...
  if args['execute']:
    if args['debug']:
      ...
      if blockCode is not None:
        curQueue=PanchayatCrawlQueue.objects.filter(panchayat__block__code=blockCode)
        step=int(args['step'])
      else:
        panchayatCode=args['panchayatCode']
      ...
      curQueue=PanchayatCrawlQueue.objects.filter( Q(isComplete=False,isError=False,status=curStep,crawlAttemptDate__lt=panchayatAttemptRetryThreshold) | Q(isError=True,isComplete=False,status=curStep,crawlAttemptDate__lt=panchayatRetryThreshold) ).order_by("-priority","crawlAttemptDate","created")[:limit]
    else:
      curQueue=PanchayatCrawlQueue.objects.filter( Q(isComplete=False,isError=False,crawlAttemptDate__lt=panchayatAttemptRetryThreshold) | Q(isError=True,isComplete=False,crawlStartDate__lt=panchayatRetryThreshold) ).order_by("-priority","created")[:limit]
      step=None
      logger.info(curQueue.first().id)
      exit(0)
    for obj in curQueue:
      qid=obj.id
      ...
      if obj.panchayat.block.district.state.isNIC == True:
        ...
        if args['startFinYear'] is not None:
          crawlPanchayat(logger,qid,step,inStartFinYear=args['startFinYear'])
        else:
          crawlPanchayat(logger,qid,step)
