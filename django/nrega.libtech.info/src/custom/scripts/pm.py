...
from nrega.models import State,District,Block,Panchayat,Applicant,Muster,WorkDetail,Wagelist,Worker
...
def populateMusterQueue(logger,q,queueSize,stateCode,panchayatCode,musterID,addLimit):
  if musterID is not None:
    ...
    myMusters=Muster.objects.filter(isDownloaded=1,panchayat__crawlRequirement="FULL",panchayat__code=panchayatCode)[:addLimit] 
...
def musterDownloadWorker(logger,q,inputargs):
  while True:
    name = threading.currentThread().getName()
    musterID = q.get()  # if there is no url, this will wait
    if musterID is None:         
      logger.info("Found Muster iD as None %s " % name)
      break
    eachMuster=Muster.objects.filter(id=musterID).first()
    logger.info("Process Muster ID : %s Thread: %s " % (str(musterID),name))
...
def main():
  ...
  if args['numberOfThreads']:
    ...
    numberOfThreads=1
  logger.info("Starting Muster Process Script with Queue Size: %s and Number of Threads: %s " % (queueSize,numberOfThreads))
  q = Queue(maxsize=queueSize)
  addLimit=queueSize-5-numberOfThreads
  populateMusterQueue(logger,q,queueSize,stateCode,panchayatCode,musterID,addLimit)
  for i in range(numberOfThreads):
    logger.info("Putting None in Muster Queue")
    q.put(None)
  time.sleep(10)
  for i in range(numberOfThreads):
    logger.info("Starting worker Thread %s " % str(i))
    t = Thread(name = 'Thread-' + str(i), target=musterDownloadWorker, args=(logger,q,args ))
    t.daemon = True
    t.start()
  time.sleep(60)
