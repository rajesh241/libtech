...
def argsFetch():
  ...
  parser.add_argument('-m', '--markTag', help='Mark that the survey is done', required=False, action='store_const', const=1)
...
def main():
    ...
    with open(filename, 'r') as f:
       ...
       for row in reader:
        ...
        if (status != "incorrectJobcard") and (status != 'dummy'):
          ...
          eachJobcard.libtechTag.add(surveyDoneTag)
          eachJobcard.save()
  ...
  if args['reportTag']:
    ...
    for inblock in blockCodes:
      ...
      i=0
      total=0
      myPanchayats=Panchayat.objects.filter(block=myBlock)[:limit]
      for eachPanchayat in myPanchayats:
        logger.info(i)
        s=''
        r=''
        myJobcards=Jobcard.objects.filter(panchayat=eachPanchayat,isBaselineSurvey=True)
        for eachJobcard in myJobcards:
          mylibtechTag=eachJobcard.libtechTag
  ...
  if args['readJobcards']:  
    filepath="./responsesq5/"
    filearray=sorted_ls(filepath)
    outfilename="q5all.csv"
#Get one file from all the files
    lines_seen = set() # holds lines already seen
    outfile = open(outfilename, "w")
    for infilename in filearray:
      for line in open(filepath+infilename, "r"):
    ...
    outcsv=''
    outcsv+="timestamp,block,panchayat,jobcard\n"
    s=''
    s+="rowNo,timestamp,block,panchayatName,PanchayatNameJobcard,jobcard\n"
    filearray=["q5all.csv"]
    for filename in filearray:
      ...
      with open(filename, 'r') as f:
         ...
         for row in reader:
          ...
          if len(row) > 10 :
            timestamp=row[0]
            blockName=row[1] #row[2]
            if blockName=="Damaragidda":
              jobcardIndex=6 #10 #6
            else:
              jobcardIndex=10 #15 #10
            tjobcard=row[jobcardIndex]
            logger.info(blockName+tjobcard)
            panchayatName=row[jobcardIndex-2]
            if len(tjobcard) != 18:
              logger.info(timestamp+blockName+tjobcard)
            else:

              surveyMarkedJobcard=Jobcard.objects.filter(tjobcard=tjobcard,libtechTag__in=surveyDoneTagArray).first()
              if surveyMarkedJobcard is None:
                eachJobcard=Jobcard.objects.filter(tjobcard=tjobcard).first()
                eachJobcard.libtechTag.add(surveyDoneTag)
                eachJobcard.save()
                outcsv+="%s,%s,%s,%s\n" % (timestamp,blockName,eachJobcard.panchayat.slug,eachJobcard.tjobcard)
                s+="%s,%s,%s,%s,%s,%s,\n" % (str(j),timestamp,blockName,panchayatName,eachJobcard.panchayat.slug,tjobcard)
              else:
                s+="%s,%s,%s,%s,%s,%s,duplicate\n" % (str(j),timestamp,blockName,panchayatName,surveyMarkedJobcard.panchayat.slug,tjobcard)

    with open("/tmp/q5surveyReportAll.csv","w") as f:
      f.write(outcsv)  
    with open("/tmp/q5debug.csv","w") as f:
      f.write(s)  
