...
def argsFetch():
  ...
  parser.add_argument('-f', '--finyear', help='StateCode for which the numbster needs to be downloaded', required=False)
...
def populateMusterQueue(logger,q,queueSize,stateCode,panchayatCode,crawlRequirement,finyear,addLimit):
  ...
  if panchayatCode is not None:
    ...
    if finyear is None:
      myMusters=Muster.objects.filter( Q(isDownloaded=False,panchayat__isnull=False,panchayat__crawlRequirement=crawlRequirement,block__district__state__isNIC=True) | Q(musterDownloadAttemptDate__lt = musterTimeThreshold,isComplete=0,isProcessed=1,panchayat__isnull=False,panchayat__crawlRequirement=crawlRequirement,block__district__state__isNIC=True) ).order_by("panchayat__block__district__state__code","downloadAttemptCount")[:addLimit] 
    else:
      myMusters=Muster.objects.filter( Q(musterDownloadAttemptDate__isnull=True,finyear=finyear,panchayat__isnull=False,panchayat__crawlRequirement=crawlRequirement,block__district__state__isNIC=True) | Q(finyear=finyear,musterDownloadAttemptDate__lt = musterTimeThreshold,isComplete=0,isProcessed=1,panchayat__isnull=False,panchayat__crawlRequirement=crawlRequirement,block__district__state__isNIC=True) ).order_by("panchayat__block__district__state__code","downloadAttemptCount")[:addLimit] 
  musterIDs=''
...
def musterQueueManager(logger,q,queueSize,stateCode,crawlRequirement,finyear):
  addLimit=queueSize-1100
  addLimit=1
  while True:
    if(q.qsize() < 1000):
      populateMusterQueue(logger,q,queueSize,stateCode,crawlRequirement,finyear,addLimit)
    else:
...
def main():
  ...
  addLimit=queueSize
  populateMusterQueue(logger,q,queueSize,stateCode,panchayatCode,crawlRequirement,finyear,addLimit)
