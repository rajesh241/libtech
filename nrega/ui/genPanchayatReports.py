...
def genReport(cur,logger,isBlock,htmlDir,finyear,districtName,blockCode,blockName,panchayatCode,panchayatName):
  ...
  query="select title,dbname,selectClause,whereClause,orderClause,dbname,groupClause,linkIndex,linkType,limitResults,finyearFilter from reportQueries where isRequired=1" 
  cur.execute(query)
