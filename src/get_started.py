...
def argsFetch():
  ...
  parser = argparse.ArgumentParser(description='Script for crawling, downloading & parsing transactions from RN6')
  parser.add_argument('-v', '--visible', help='Make the browser visible', required=False, action='store_const', const=True)
  parser.add_argument('-l', '--log-level', help='Log level defining verbosity', required=False)
  parser.add_argument('-t', '--timeout', help='Time to wait before a page loads', required=False)
  parser.add_argument('-b', '--browser', help='Specify the browser to test with', required=False)
  parser.add_argument('-d', '--directory', help='Specify the directory to download HTMLs to', required=False)
  parser.add_argument('-u', '--url', help='Specify the url to crawl', required=False)
  parser.add_argument('-p', '--parse', help='Parse the HTML files', required=False, action='store_const', const=True)
