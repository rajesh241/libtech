...
def main():
  ...
  query="use libtech"
  cur.execute(query)
  query="select bid,type,minhour,maxhour,tfileid,fileid,groups,vendor,district,blocks,panchayats,priority,fileid2,template,inQuery from broadcasts where error=0 and approved=1 and processed=0 and startDate <= CURDATE();"
  print(query)
  cur.execute(query)
  ...
  for row in results:
    bid=str(row[0])
    template=row[13]
    if((template == 'general') or  (template == 'feedback') or  (template == 'biharVoiceMail') or (template == 'ghattuFeedback') or (template == 'rscdFeedback') ):
