...
#    query="select b.name,b.blockCode,p.name,p.panchayatCode from panchayats p, blocks b where b.blockCode=p.blockCode and p.isRequired=1 and b.blockCode='003' and panchayatCode='013'"
    ...
    for row in results:
      ...
      query="select title,dbname,selectClause,whereClause,orderClause,dbname,groupClause,linkIndex,linkType,limitResults from reportQueries" 
      cur.execute(query)
      ...
      for queryRow in queryResults:
        ...
        if groupClause is None:
          query=selectClause+" where "+whereClause+blockFilterQuery+panchayatFilterQuery+"  order by  "+orderClause+limitString
        else: 
          query=selectClause+" where "+whereClause+blockFilterQuery+panchayatFilterQuery+" group by "+groupClause+"  order by  "+orderClause+limitString
        ...
        myhtml+= queryTable
#
        curhtmlfile=htmlDir+districtName.upper()+"/"+blockName.upper()+"/"+panchayatName.upper()+"/REPORTS/"+title.replace(' ','')+".html"
        ...
        query=selectClause+" where "+whereClause+blockFilterQuery+panchayatFilterQuery+"  order by  "+orderClause
        logger.info(query)
        curcsvfile=htmlDir+districtName.upper()+"/"+blockName.upper()+"/"+panchayatName.upper()+"/REPORTS/"+title.replace(' ','')+".csv"
