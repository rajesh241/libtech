...
#    query="select b.name,b.blockCode,p.name,p.panchayatCode from panchayats p, blocks b where b.blockCode=p.blockCode and p.isRequired=1 and b.blockCode='003' and panchayatCode='013'"
    ...
    for row in results:
      ...
      logger.info("Processing"+blockName) 
      ...
      for queryRow in queryResults:
        ...
        myhtml+= queryTable
#
        curhtmlfile=htmlDir+districtName.upper()+"/"+blockName.upper()+"/REPORTS/"+title.replace(' ','')+".html"
        ...
        logger.info(query)
        curcsvfile=htmlDir+districtName.upper()+"/"+blockName.upper()+"/REPORTS/"+title.replace(' ','')+".csv"
